...
function renderBets(){
  ...
  else { V={b:'🔴 見送り推奨',c:'#e74c3c',bg:'#3a1a1a',r:'軸'+um[A]+'番は想定'+(_srcA<99?_srcA+'番人気':'-')+'＝市場の中心で単勝オッズに妙味が乏しい（堅いが複勝頼みで本線の旨みは薄い）'}; }
  ...
  var col2=contend.filter(function(n){ return n!==A && P2(n)>=0.18; }).sort(function(a,b){return P2(b)-P2(a);});
  if(col2.length<2) col2=contend.filter(function(n){return n!==A;}).sort(function(a,b){return P2(b)-P2(a);}).slice(0,2);
  col2=col2.slice(0,5);
  var col3=contend.filter(function(n){ return n!==A && P3(n)>=0.18; }).sort(function(a,b){return P3(b)-P3(a);});
  if(col3.length<3) col3=contend.filter(function(n){return n!==A;}).sort(function(a,b){return P3(b)-P3(a);}).slice(0,3);
  col3=col3.slice(0,6);
  // info & prob table
