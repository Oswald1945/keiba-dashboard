...
function renderBets(){
  ...
  col2=col2.slice(0,5);
  var col3=contend.filter(function(n){ return _ex(n) && P3(n)>=0.18; }).sort(function(a,b){return P3(b)-P3(a);});
  if(col3.length<3) col3=contend.filter(_ex).sort(function(a,b){return P3(b)-P3(a);}).slice(0,3);
  col3=col3.slice(0,6);
  // 表示は各列とも馬番(若い順)に並べる
